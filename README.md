...
I spent quite some time setting up my Raspberry Pi's, so backup is really important.
...
Then it gzips everything, encrypt it with GPG, and uploads to dropbox, after which gzip and gpg encrypted files are removed (not the rsync'd folders because I want to keep them)
